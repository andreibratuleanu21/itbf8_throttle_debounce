...
var socket = new WebSocket('ws://localhost:8080'); // RAW MOVE EV
...
function processMoveEv(ev) {
  socket.send("".concat(ev.x, ":").concat(ev.y - 80));
  counter++;
}
...
socket.onmessage = function (event) {
  ...
  var element = document.querySelector("#content > #".concat(data[0]));

  if (element) {
    element.style.left = "".concat(data[1], "px");
    element.style.top = "".concat(data[2], "px");
  } else {
    ...
    newElement.style.left = "".concat(data[1], "px");
    newElement.style.top = "".concat(data[2], "px");
